...
####################################### Scan Function ###########################################
def scan_ip(target_ip, username, password, timeout):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return target_ip, "Login Fail", "N/A"
...
def main():
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor: #using ThreadPoolExecutor for faster scan
            results=executor.map(lambda ip: scan_ip(ip,username,password,timeout),ip_range)
